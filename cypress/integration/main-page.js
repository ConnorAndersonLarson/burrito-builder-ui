describe('Main-Page Testing', () => {
  ...
  it('Should give errors if submission is incomplete', () => {
    cy.get('[name=beans]').click()
      .get('#submitOrder').click()
      .get('#ingErr').should('contain', 'Please enter a name')
    cy.visit(baseURL)
    cy.get('[type=text]').type('Reginald')
      .get('#submitOrder').click()
      .get('#ingErr').should('contain', 'Please enter at least one ingredient')
    cy.visit(baseURL)
    cy.get('#submitOrder').click()
      .get('#ingErr').should('contain', 'Please enter a name and at least one ingredient')
  })
})
